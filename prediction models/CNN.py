...
#matrix prep
train_matrix_x = full_matrix[0:3]
val_matrix_x = full_matrix[3:4]
print(train_matrix_x.shape)
test_matrix_x = full_matrix[4:5]

train_matrix_x = np.repeat(train_matrix_x[:, np.newaxis, :, :, :], 12, axis=1)
val_matrix_x = np.repeat(val_matrix_x[:, np.newaxis, :, :, :], val_tabular_x.shape[0]/full_matrix.shape[1], axis=1)
...
#to save model
model.save('current_best_19_val.keras')

# to load previous model
from keras.models import load_model
model = load_model('current_best_19_val.keras')
...
pyreadr.write_rds("D:/consulting/code_consulting/Africa/python_pred_results/19val_predictions.rds", test_tabular)
